...
export default function (pi: ExtensionAPI) {
	...
	function spawnAgent(state: SubState, prompt: string, ctx: any): Promise<void> {
		...
		return new Promise<void>((resolve) => {
			...
			proc.stderr!.on("data", (chunk: string) => {
				if (chunk.trim()) { state.textChunks.push(chunk); updateWidgets(); }
			});
	...
	pi.registerTool({
		...
		description: "Spawn a background subagent to perform a task. Returns immediately; result delivered as a follow-up message when done.",
		...
		execute: async (_callId, args, _signal, _onUpdate, ctx) => {
			widgetCtx = ctx;
			const id = nextId++;
			const state: SubState = { id, status: "running", task: args.task, textChunks: [], toolCount: 0, elapsed: 0, sessionFile: makeSessionFile(id), turnCount: 1 };
			agents.set(id, state);
			updateWidgets();
			spawnAgent(state, args.task, ctx);
			return { content: [{ type: "text", text: `Subagent #${id} spawned.` }] };
		},
	});

	pi.registerTool({
		name: "subagent_continue",
		description: "Continue an existing subagent's conversation with a follow-up prompt.",
		parameters: Type.Object({
		...
		execute: async (_callId, args, _signal, _onUpdate, ctx) => {
			...
			if (!state) return { content: [{ type: "text", text: `No subagent #${args.id}.` }] };
			if (state.status === "running") return { content: [{ type: "text", text: `Subagent #${args.id} still running.` }] };
			state.status = "running"; state.task = args.prompt; state.textChunks = []; state.elapsed = 0; state.turnCount++;
			updateWidgets();
	...
	pi.registerTool({
		name: "subagent_list",
		description: "List all subagents with their IDs, tasks, and status.",
		parameters: Type.Object({}),
		execute: async () => {
			if (agents.size === 0) return { content: [{ type: "text", text: "No subagents." }] };
			const list = Array.from(agents.values()).map(s => `#${s.id} [${s.status.toUpperCase()}] Turn ${s.turnCount} — ${s.task}`).join("\n");
	...
	pi.registerTool({
		...
		description: "Remove a subagent. Kills it if still running.",
	...
	pi.registerCommand("sub", {
		...
		handler: async (args, ctx) => {
			...
			const id = nextId++;
			const state: SubState = { id, status: "running", task, textChunks: [], toolCount: 0, elapsed: 0, sessionFile: makeSessionFile(id), turnCount: 1 };
			agents.set(id, state);
			updateWidgets();
	...
	pi.registerCommand("subcont", {
		...
		handler: async (args, ctx) => {
			...
			if (!state) { ctx.ui.notify(`No subagent #${num}.`, "error"); return; }
			if (state.status === "running") { ctx.ui.notify(`Subagent #${num} still running.`, "warning"); return; }
			state.status = "running"; state.task = prompt; state.textChunks = []; state.elapsed = 0; state.turnCount++;
			updateWidgets();
	...
	pi.registerCommand("subclear", {
		...
		handler: async (_args, ctx) => {
			...
			ctx.ui.notify(total === 0 ? "No subagents." : `Cleared ${total}${killed > 0 ? ` (${killed} killed)` : ""}.`, total === 0 ? "info" : "success");
		},
	});
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		agents.clear(); nextId = 1; widgetCtx = ctx;
	});
}
