 ...
 *
 * Line 1: model + context meter (left), tokens in/out + cost (right)
 * Line 2: cwd + branch (left), per-tool call tally (right)
 *
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					// Accumulate token + cost totals from session branch
					let tokIn = 0;
					let tokOut = 0;
					let cost = 0;
